...
public interface SequencedMerge<T> extends FairMerge<List<T>> {
    ...
    default <S extends T> List<T> subtypeMerge(List<S> l1, List<S> l2, boolean selfMerge) {
        ...
        return merge(copy1, copy2, selfMerge);
    }
    /**
    ...
    default List<T> merge(List<T> l1, List<T> l2, boolean selfMerge) {
        ...
        for(OverallSequencedMergeProperty<T> overallProperty : generateOverallProperties(l1, l2)) {
            if(overallProperty.isSuitable()) {
                // Overall properties are capable of creating a full result by themselves
                overallProperty.blendResult(finalResult);
                return finalResult;
            }
        }
        ...
        for (T element : l1) {
            ...
            for (T element2 : l2) {
                ...
                if (optionalMergeData.isPresent()) {
                    ...
                    mergeData.blendResult(finalResult);
                    l1.set(i, null);
                    l2.set(j, null);
