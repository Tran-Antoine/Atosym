...
/**
 * A behavior that takes two entries of the same type, and outputs a result that may or may not be of the same type. <p>
 ...
 * matches the two inputs, an error is automatically thrown. <p>
 * If there is only one property that can be applied to any pair of input, use {@code Collections.singletonList(property)}. <p>
 *
...
public interface Merge<P, R, PROP extends OverallMergeProperty<P, R>> {
    ...
    /**
     * Computes a result from the two input given. <p>
     ...
     * {@link OverallMergeProperty#isSuitable()}. The boolean parameter is only used through redefinitions of this
    ...
    /**
     * Generates a list of properties from the given input. <p>
